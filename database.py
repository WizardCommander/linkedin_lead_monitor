...
def save_activity_id(platform: str, activity_id: str) -> bool:
    """Save an activity ID to track discovered posts"""
    try:
        with sqlite3.connect(DB_NAME) as conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT OR IGNORE INTO activity_ids (platform, activity_id, discovered_at)
                VALUES (?, ?, ?)
            """, (platform, activity_id, datetime.now().isoformat()))
            return cursor.rowcount > 0
    except sqlite3.IntegrityError:
        return False
...
def get_unscraped_activity_ids(platform: str) -> List[str]:
    """Get activity IDs that haven't been scraped yet"""
    with sqlite3.connect(DB_NAME) as conn:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT activity_id FROM activity_ids
