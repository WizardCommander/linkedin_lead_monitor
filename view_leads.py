...
def compare_platforms():
    ...
    print(
        f"  Avg content len: {sum(len(l.get('post_content', '')) for l in linkedin_leads) // max(len(linkedin_leads), 1)} chars"
    )
    ...
    print(
        f"  Avg content len: {sum(len(l.get('post_content', '')) for l in bluesky_leads) // max(len(bluesky_leads), 1)} chars"
    )
